...
describe('QueryManager', function () {
    ...
    var mockRefetch = function (_a) {
        ...
        return mockQueryManager_1.default.apply(void 0, args);
    };
            ...
            next: function (result) {
                try {
                    switch (count++) {
                        ...
                        case 1:
                            chai_1.assert.deepEqual(result.data, data2);
                            chai_1.assert.notStrictEqual(result.data, firstResultData);
                            chai_1.assert.strictEqual(result.data.b, firstResultData.b);
                            chai_1.assert.notStrictEqual(result.data.d, firstResultData.d);
                            chai_1.assert.strictEqual(result.data.d.f, firstResultData.d.f);
                            observable.refetch();
                            break;
                        case 2:
    ...
    it('will return referentially equivalent data in getCurrentResult if nothing changed', function (done) {
        ...
        observable.subscribe({
            next: function (result) {
                try {
                    chai_1.assert.deepEqual(result.data, data1);
                    chai_1.assert.strictEqual(result.data, observable.currentResult().data);
                    done();
                }
                catch (error) {
                    done(error);
    ...
    it('deepFreezes results in development mode', function () {
        var query = (_a = ["{ stuff }"], _a.raw = ["{ stuff }"], graphql_tag_1.default(_a));
        ...
        return queryManager.query({ query: query })
            ...
            chai_1.assert.throws(function () { return result.data.stuff = 'awful'; });
        });
        var _a;
    });
    ...
    it('does not call broadcastNewStore when Apollo state is not affected by an action', function () {
        ...
        return observableToPromise_1.default({ observable: observable }, function (result) {
            ...
            chai_1.assert.equal(store.getState().test, true, 'test state should have been updated');
            chai_1.assert.equal(called, false, 'broadcastNewStore should not have been called');
        });
        var _a;
    });
